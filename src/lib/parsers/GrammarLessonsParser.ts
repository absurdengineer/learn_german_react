...
import { BaseCSVParser, DataStandardizer, type StandardizedGrammarLesson } from './DataLoader';

export class GrammarLessonsParser extends BaseCSVParser<StandardizedGrammarLesson> {
  protected csvContent = grammarLessonsCSV;

  protected parseRow(data: Record<string, string>, _index: number): StandardizedGrammarLesson | null {
  ...
  private extractCategory(title: string, content: string): string {
    ...
    if (titleLower.includes('greeting') || contentLower.includes('grüße')) return 'greetings';
    if (titleLower.includes('introduction') || contentLower.includes('introduction')) return 'introductions';
    if (titleLower.includes('number') || contentLower.includes('zahlen')) return 'numbers';
    if (titleLower.includes('verb') || contentLower.includes('verb')) return 'verbs';
    if (titleLower.includes('noun') || contentLower.includes('noun')) return 'nouns';
    if (titleLower.includes('article') || contentLower.includes('artikel')) return 'articles';
    if (titleLower.includes('case') || contentLower.includes('case')) return 'cases';
    if (titleLower.includes('alphabet') || contentLower.includes('alphabet')) return 'alphabet';
