...
import {
  BaseCSVParser,
  DataStandardizer,
  type StandardizedGrammarPractice,
} from "./DataLoader";
...
export class GrammarPracticeParser extends BaseCSVParser<StandardizedGrammarPractice> {
  ...
  protected parseRow(
    ...
    index: number
  ): StandardizedGrammarPractice | null {
  ...
  private normalizeQuestionType(
    type: string
  ): "multiple_choice" | "fill_in_blank" | "true_false" | "ordering" {
    const normalized = type.toLowerCase().trim();
    if (normalized.includes("multiple") || normalized.includes("choice"))
      return "multiple_choice";
    if (normalized.includes("fill") || normalized.includes("blank"))
  ...
  private extractCategoryFromPrompt(prompt: string): string {
    ...
    if (promptLower.includes("greet") || promptLower.includes("hallo"))
      return "greetings";
    if (promptLower.includes("alphabet") || promptLower.includes("letter"))
      return "alphabet";
    if (
      promptLower.includes("article") ||
      promptLower.includes("der") ||
      promptLower.includes("die") ||
      promptLower.includes("das")
    )
      return "articles";
    if (promptLower.includes("verb") || promptLower.includes("conjugat"))
      return "verbs";
    if (promptLower.includes("noun") || promptLower.includes("gender"))
      return "nouns";
    if (promptLower.includes("number") || promptLower.includes("zahl"))
      return "numbers";
    if (
      promptLower.includes("pronoun") ||
      promptLower.includes("ich") ||
      promptLower.includes("du")
    )
      return "pronouns";
    if (
      promptLower.includes("case") ||
      promptLower.includes("accusative") ||
      promptLower.includes("nominative")
    )
      return "cases";
  ...
  private calculateDifficulty(dayReference: number, prompt: string): number {
    ...
    const promptLower = prompt.toLowerCase();
    if (promptLower.includes("basic") || promptLower.includes("simple"))
      difficulty = Math.max(1, difficulty - 1);
    if (promptLower.includes("complex") || promptLower.includes("advanced"))
      difficulty += 1;
    if (promptLower.includes("case") || promptLower.includes("conjugat"))
      difficulty += 1;
