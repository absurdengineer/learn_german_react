          ...
          switch (type) {
            ...
            case "multiple-choice-de-en":
              ...
              wrongOptions = getRandomVocabularyWords(3, [word.german])
                ...
                .filter(
                  (option) =>
                    option && option.trim() !== "" && option !== correctAnswer
                )
                .slice(0, 3);
              break;
            case "translation-en-de":
  ...
  const handleQuizComplete = (results: QuizResults) => {
    const sessionResult: SessionResult = {
      ...
      mistakes: results.mistakes.map((mistake: QuizMistake) => {
        // Find the word based on the question content
        let word = mistake.word;
        if (!word) {
          // For German → English: question contains German word, correctAnswer is English
          if (sessionType.includes("de-en")) {
            word =
              sessionWords.find(
                (w) =>
                  w.german ===
                  mistake.prompt.replace(
                    /What is the English translation of "(.+)"\?/,
                    "$1"
                  )
              ) || sessionWords[0];
          }
          // For English → German: question contains English word, correctAnswer is German
          else if (sessionType.includes("en-de")) {
            word =
              sessionWords.find(
                (w) =>
                  w.english ===
          ...
          // Fallback: try to find by correct answer
          else {
            word =
              sessionWords.find(
                (w) =>
                  w.german === mistake.correctAnswer ||
                  w.english === mistake.correctAnswer
              ) || sessionWords[0];
          }
        }
  ...
  const handleFlashcardSessionComplete = (results: FlashcardSessionResult) => {
    const sessionResult: SessionResult = {
      ...
      mistakes: results.mistakes.map((mistake) => ({
        word:
          sessionWords.find((w) => w.id.value === mistake.item.id) ||
          sessionWords[0],
