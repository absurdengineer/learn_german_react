...
const PracticeSessionResults: React.FC<PracticeSessionResultsProps> = ({
  results,
  sessionType,
  onRestart,
  onReviewMistakes,
  ...
  const timePerQuestion = totalQuestions > 0 ? (timeSpent / totalQuestions / 1000).toFixed(1) : 0;
                ...
                <ul className="space-y-4">
                  {mistakes.map((mistake, index) => (
                    <li key={index} className="p-4 bg-white rounded-lg shadow-md">
                      <p className="font-semibold text-lg text-gray-800">{mistake.word.german}</p>
                      <p className="text-sm text-gray-600">({mistake.word.english})</p>
