...
const TestResults: React.FC = () => {
  ...
  const { userAnswers, test, result } = (state as { 
    userAnswers: { [key: string]: string }; 
    test: Test; 
    result: TestResult 
  }) || {};
  ...
  const timePerQuestion = result.totalQuestions > 0 ? (result.totalTime / 1000 / result.totalQuestions).toFixed(1) : 0;
      ...
      <div className="max-w-3xl mx-auto">
        <div className="bg-white shadow-2xl rounded-2xl p-8">
          <div className="text-center">
            ...
            <h1 className="text-4xl font-extrabold text-gray-900">Test Complete!</h1>
            <p className="mt-2 text-lg text-gray-600">
              You've completed the <span className="font-semibold text-indigo-600">{result.title}</span> test.
            </p>
          ...
          <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
            <div className={`border rounded-xl p-6 ${
              ...
              }`}>{accuracy.toFixed(0)}%</p>
            </div>
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
              <p className="text-sm font-medium text-blue-600">Correct</p>
              <p className="mt-2 text-4xl font-bold text-blue-800">{result.score}</p>
            </div>
            <div className="bg-red-50 border border-red-200 rounded-xl p-6">
              <p className="text-sm font-medium text-red-600">Incorrect</p>
              <p className="mt-2 text-4xl font-bold text-red-800">{result.totalQuestions - result.score}</p>
            </div>
          </div>
          ...
          {incorrectAnswers.length > 0 && (
            <div className="mt-10">
              <h2 className="text-2xl font-bold text-gray-900 text-center mb-6">Review Incorrect Answers</h2>
              <div className="bg-gray-50 rounded-xl p-6 max-h-80 overflow-y-auto">
                <ul className="space-y-6">
                  {incorrectAnswers.map((question) => (
                    <li key={question.id} className="border-b border-gray-200 pb-4 last:border-b-0">
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                          <div>
                            ...
                            <span className="text-red-700">{userAnswers[question.id] || 'No answer'}</span>
                          </div>
                          <div>
          ...
          <div className="mt-12 flex flex-col sm:flex-row justify-center gap-4">
            <button
              onClick={() => navigate('/tests')}
              className="w-full sm:w-auto bg-indigo-600 text-white py-3 px-6 rounded-lg hover:bg-indigo-700 transition-transform transform hover:scale-105 font-semibold"
            >
              Take Another Test
            </button>
            <button
              onClick={() => navigate('/')}
              className="w-full sm:w-auto bg-gray-600 text-white py-3 px-6 rounded-lg hover:bg-gray-700 transition-transform transform hover:scale-105 font-semibold"
            >
              Back to Home
            </button>
          </div>
